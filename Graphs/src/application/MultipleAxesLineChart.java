...
public class MultipleAxesLineChart extends StackPane {
	...
	public MultipleAxesLineChart(LineChart baseChart, Color lineColour) {
		this(baseChart, lineColour, null);
	}
	
	public MultipleAxesLineChart(LineChart baseChart, Color lineColour, Double strokeWidth) {
		if (strokeWidth != null)
			this.strokeWidth = strokeWidth;
		this.baseChart = baseChart;
		
		chartColorMap.put(baseChart, lineColour);
		
		styleBaseChart(baseChart);
		styleChartLine(baseChart, lineColour);
	...
	public void addSeries(XYChart.Series series, Color lineColour) {
		...
		styleBackgroundChart(lineChart, lineColour);
		setFixedAxisWidth(lineChart);
		
		chartColorMap.put(lineChart, lineColour);
		backgroundCharts.add(lineChart);
	}
	
	private void styleBackgroundChart(LineChart lineChart, Color lineColour) {
		styleChartLine(lineChart, lineColour);
	...
	private String toRGBCode(Color colour) {
	...
	private void styleChartLine(LineChart chart, Color lineColour) {
		chart.getYAxis().lookup(".axis-label").setStyle("-fx-text-fill: " + toRGBCode(lineColour) + "; -fx-font-weight: bold;");
		Node seriesLine = chart.lookup(".chart-series-line");
		seriesLine.setStyle("-fx-stroke: " + toRGBCode(lineColour) + "; -fx-stroke-width: "+ strokeWidth + ";");
	}
